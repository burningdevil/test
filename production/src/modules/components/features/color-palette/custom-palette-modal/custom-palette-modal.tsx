...
import { selectAllColorPalettes, selectApplicationDefaultPalette, selectApplicationPalettes, selectUseConfigPalettes } from "../../../../../store/selectors/HomeScreenConfigEditorSelector";
...
const ColorPaletteBlade: React.FC<any> = (props: any) => {
  ...
  const checkIndeterminate = (selectedCount: number, total: number, selectedRows?: any[]) => {
  ...
  const handleSaveAdd = () => {
    ...
    const defaultApplicationPalettes = applicationPalettes.filter(v => paletteList.filter(v => v.paletteType === 1).map(v => v.id).includes(v));
                ...
                <div className={`custom-palette-modal`}>
                    ...
                    <div className={`${classNamePrefix}-top`}>
                        ...
                        <div className={`${classNamePrefix}-search-container`}>
                            <SearchInput className={`${classNamePrefix}-search`} placeholder={localizedStrings.SEARCH} value={nameFilter}
                            ...
                            <div className = {`${classNamePrefix}-add-palette`}>
                                ...
                                <ColorPaletteEditor visible = {isShowEditPalette} isCreate = {true} onClose ={() => setEditorPalette(false)}></ColorPaletteEditor>
                            </div>
                        </div>
                    ...
                    <div className={`${classNamePrefix}-middle`}>
                        ...
                        <div className={`${classNamePrefix}-grid-right`}>
                            <CustomPaletteModalGrid nameFilter = {nameFilter} paletteType={2} checkIndeterminate = {checkIndeterminate} setCustomPalettes = {setCustomPalettes} setPaletteLength = {setPaletteLength}></CustomPaletteModalGrid>
                        </div>
                        </div>

...
const CustomPaletteModalGrid: React.FC<PaletteProps> = (props: PaletteProps) => {
  const {
    setPaletteLength,
    nameFilter,
    paletteType,
  ...
  useEffect(() => {
    if (nameFilter) {
      const filterList = dataSource.filter(
        (v) => v.name.toLowerCase().indexOf(nameFilter.toLowerCase()) > -1
      );
  ...
  const [columns, setColumns] = useState(
    getColumns(isDefaultPalette, dispatch, currentList, setCurrentList, false)
  );

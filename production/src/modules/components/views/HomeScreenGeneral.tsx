...
class HomeScreenGeneral extends React.Component<any, any> {
    ...
    componentDidUpdate(prevProps: any) {
        if (
            !this.state.isDefaultNameFocused &&
            (!this.props.config.id || this.props.isDuplicateConfig) &&
        ...
        if (
            !this.state.isNameChangedByManual &&
            prevProps.config.name !== this.props.config.name
        ) {
            this.setState({
    ...
    validateName(name: string) {
        ...
        const showNameError =
            isEmptyName || isDuplicateName || isInvalidCharacter;
        let nameErrorMsg = '';
        if (isEmptyName) {
    ...
    handlePreViewDeviceTypeInvalid = (platforms: string[]) => {
        ...
        const availableTypes = _.concat(
            platforms.includes(platformType.mobile)
                ? [reviewType.TABLET, reviewType.PHONE]
                : [],
            platforms.includes(platformType.web) ? reviewType.WEB : [],
            platforms.includes(platformType.desktop) ? reviewType.DESKTOP : []
        );
    ...
    render() {
        ...
        const isShowAuthMode = this.props.authModeEnable;
        return (
                    ...
                    >
                        <Input
                            ...
                            onValidate={(e: string) => {
                                ...
                                if (!this.state.isNameChangedByManual)
                                    return true;

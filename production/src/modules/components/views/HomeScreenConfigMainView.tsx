...
import { isLibraryServerVersionMatch, isIServerVersionMatch, isUserHasManageApplicationPrivilege, APPLICATIONS_FOLDER_ID, APPLICATIONS_FOLDER_TYPE, LIBRARY_SERVER_SUPPORT_DOC_TYPE_VERSION, LIBRARY_SERVER_SUPPORT_CONTENT_GROUP_VERSION, isUserHasManageContentGroupPrivilege } from '../../../utils';
...
class HomeScreenConfigMainView extends React.Component<any, any> {
  ...
  checkServerAndUserPrivilege = async () => {
    ...
    if (isConnected) {
      ...
      const isLibraryVersionSupportContentGroup = !!status.webVersion && isLibraryServerVersionMatch(status.webVersion, LIBRARY_SERVER_SUPPORT_CONTENT_GROUP_VERSION) && isUserHasManageContentGroupPrivilege(currentEnv.privileges)

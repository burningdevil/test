...
export default class MetricEditorPage extends BasePage {
  ...
  getNavigationContainer() {
    return this.getMetricEditorWindow().$(
      '.mstrmojo-Box.mstrmojo-MetricIDE-browsers-container'
    )
  }

  getContentContainer() {
    return this.getMetricEditorWindow().$(
      '.mstrmojo-Box.mstrmojo-MetricIDE-editors-container'
    )
  }

  getFormulaContainer() {
    return this.getContentContainer().$(
      '.mstrmojo-Editor.mstrmojo-MetricEditor.mstrmojo-MetricIDE-editor.inline'
    )
  }

  getFunctionContainer() {
    return this.getContentContainer().$(
      '.mstrmojo-Editor.mstrmojo-Editor-me.mstrmojo-SimpleMetricEditor.mstrmojo-MetricIDE-editor.inline'
    )
  }

  getMetricNameInputBox() {
    return this.getFormulaContainer().$(
  ...
  getFormulaButtonContainer() {
    return this.getFormulaContainer()
      ...
      .$('.mstrmojo-HBox.mstrmojo-ME-buttonBox')
  }

  getFunctionButtonContainer() {
    return this.getFunctionContainer()
      .$('.mstrmojo-Editor-buttons')
      .$('.mstrmojo-HBox.mstrmojo-ME-buttonBox')
  }
  ...
  getButtons(buttonName) {
    return this.getToolbarHeader()
      .$$('.toolbar-button')
      .filter(async elem => {
        const button = await elem.$('.control-label.center-display').getText()
  ...
  getPopUpButtons(buttonName) {
    return this.getConfirmationPopWindow()
      .$$('.mstrmojo-HBox-cell.subBox')
      .filter(async elem => {
        const button = await elem.getText()
        return button === buttonName
      })
      .first()
  }
  ...
  getFunctionSelector() {
    return this.$(
      '.mstrmojo-ListBase2-itemsContainer.hasVertical.mstrmojo-scrollNode'
    )
  }

  getEditorFunction(functionName) {
    return this.getFunctionSelector()
      .$$('.mstrmojo-itemwrap')
      .filter(async elem => {
        const func = await elem.$('.mstrmojo-suggest-text.fn').getText()
  ...
  getFormulaExpressionToolbar() {
    return this.getContentContainer().$(
      '.mstrmojo-ListBase2.mstrmojo-ExpressionToolbar'
    )
  }

  getFunctionEditorOption(option) {
    return this.getFormulaExpressionToolbar()
      .$$('.mstrmojo-itemwrap')
      .filter(async elem => {
        const options = await elem.getText()
  ...
  getValidateButton() {
    return this.getFormulaStatusBox().$(
      '.mstrmojo-Button.mstrmojo-WebHoverButton.right.hoverLink'
    )
  }
  ...
  async clearFormulaEditor(option) {
    return this.getFunctionEditorOption(option)
      .$('.mstrmojo-Button.mstrmojo-WebHoverButton.clear.right.hoverLink')
      .click()
  }
  ...
  async clickValidate() {
    ...
    return this.brwsr.wait(
      this.EC.presenceOf(this.getFormulaStatus()),
      5000,
  ...
  async provideMetricName(metricName) {
    ...
    await this.getMetricNameInputBox().sendKeys(
      protractor.Key.HOME,

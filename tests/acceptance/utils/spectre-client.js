// copy of https://github.com/fionnbharra/spectre_js_client/, added `diffThreshold` option support
const rp = require('request-promise');

const missingParam = (...params) => params.indexOf(undefined) > -1; // eslint-disable-line no-undefined

module.exports = (projectName, suiteName, urlBase) => {
    if (missingParam(projectName, suiteName, urlBase)) {
        throw new Error('Missing parameter');
    }

    let runId;

    const submitTestOptions = options => {
        return {
            method: 'POST',
            uri: `${urlBase}/tests`,
            ...
            formData: {
                ...
                'test[diff_threshold]': options.diffThreshold || ''
            }
        ...
        connection = connectionOptions => {
            return rp(connectionOptions)
                .then(spectreResponse =>
                    spectreConnection(JSON.parse(spectreResponse))
                )
                .catch(err => new Error(err));
        },
        connectionOptions = {

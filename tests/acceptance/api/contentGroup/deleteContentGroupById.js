...
import { errorLog, successLog, infoLog } from '../../config/consoleFormat';

export default async function deleteContentGroupById({ baseUrl, session, contentGroupId }) {
            ...
            if (!error) {
                if (response.statusCode === 204) {
                    ...
                    if (bodyInfo.code === 'ERR004' && bodyInfo.iServerCode === -2147216373) {
                        ...
                        errorLog(`Delete Content Group '${contentGroupId}' failed. Status code: ${response.statusCode}. Message: ${bodyInfo.message}`);
                        reject(bodyInfo);
                    }
                } else {
                    errorLog(`Delete Content Group '${contentGroupId}' failed. Status code: ${response.statusCode}. Message: ${body.message}`);
                    reject(body);
                }
            } else {

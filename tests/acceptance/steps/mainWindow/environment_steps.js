...
When(
  "I login with {loginMode} mode using name {userName} and password {userPwd}",
  async function(loginMode, userName, userPwd) {
    if (!mainWindow.mainCanvas.envSection.isEnvAdded(envName)) {
      if (userPwd === "blank") userPwd = "";
      return mainWindow.mainCanvas.envSection.loginToEnv(
        loginMode,
        userName,
        userPwd
      );
...
Then("I disconnect environment", async function() {
  return mainWindow.mainCanvas.envSection.disconnectEnv(
    browser.params.envInfo[0].envName
  );
});
...
Then("{envName} is connected", async function(envName) {
  return expect(mainWindow.mainCanvas.envSection.isEnvAdded(envName)).become(
    true
  );
});

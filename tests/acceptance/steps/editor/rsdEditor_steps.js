...
Then(/^The "([^"]*)" RSD window should be present$/, async function (windowName) {
    await rsdWindow.waitRSDWindow(windowName);
});

When(/^I select the "([^"]*)" in create new document window$/, async function (rsdTemplate) {
  await rsdPage.clickRSDTemplateIcon(rsdTemplate);
  return rsdPage.switchToNewWebView();
});

Then(/^The RSD scale setting with "([^"]*)" should be displayed$/, async function (percentage) {
...
When(/^I change the scale from "([^"]*)" to "([^"]*)"$/, async function (fromPercentage, toPercentage) {
  await rsdPage.selectDropdownSelection(fromPercentage);
  return rsdPage.selectDropdownSelection(toPercentage);
});

When(/^I close the "([^"]*)" RSD Window$/, async function (windowName) {
  await rsdWindow.closeWindow(windowName);
  return rsdPage.switchToDefaultWebView();
});

When(/^I directly close the "([^"]*)" RSD Window$/, async function (windowName) {
  await rsdWindow.closeWindow(windowName);
  if (OSType === 'windows') {
